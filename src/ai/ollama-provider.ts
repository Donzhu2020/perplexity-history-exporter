...
export class OllamaProvider implements AIProvider {
  ...
  readonly embeddingModel = config.ollamaEmbedModel
  readonly generationModel = config.ollamaModel

  async embed(texts: string[], _taskType: EmbeddingTaskType = 'document'): Promise<number[][]> {
  ...
  async generate(prompt: string, modelOverride?: string): Promise<string> {
    const requestBody = {
      model: modelOverride ?? this.generationModel,
      prompt,
      stream: false,
    }
  ...
  async validate(): Promise<void> {
    ...
    try {
      ...
      throw new OllamaProvider.OllamaError(`Ollama validation failed: ${message}`)
    }
  }
  ...
  private async performOllamaHttpRequest(endpoint: string, body: object): Promise<unknown> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new OllamaProvider.OllamaError(
      ...
      if (_error instanceof OllamaProvider.OllamaError) throw _error
      throw new OllamaProvider.OllamaError(
